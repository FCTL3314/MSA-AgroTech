...
рамках фермы: путь от камеры до сирены не выходит за локальную сеть, поэтому выполняем НФТ 3, 4 и 6. В центральный сервер уходят события и метрики, видео не уходит.
...
Разрабатываем:
Агента фермы и центральную платформу. IoT-платформу на ферму не берём, без центра она не работает.
SCADA теплиц не подходит, это другой домен.

**Плюсы:**
- Тяжелая работа выполняется целиком внутри локальный сети, не требуется отправлять тяжеловесные данные по сети.
- В центр идут события вместо видео, поэтому задержка до 10 минут и досылка накопленного после обрыва не мешают, выполняем НФТ 5.
- Агенты одинаковые, подключение фермы не требует проводить доработку центра. Выполняем ФТ 9 и НФТ 2.
- Отказ одной фермы локален и не валит всю систему целиком. А так же при недоступности центра ферма продолжает работать. Выполняем НФТ 1.
...
- Одну систему эксплуатировать проще, чем множество распределённых серверов
...
- Для SaaS пришлось бы заводить трафик чужих ферм во внутреннюю сеть компании. Тратим вычислительные мощности на работу чужих ферм.
